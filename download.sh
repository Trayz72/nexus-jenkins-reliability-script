...
BACKEND_URL=$(curl -u "$USER:$PASS" -s -X GET \
"${URL}?repository=spring-boot-repo&maven.extension=jar" \
| grep -Po '"downloadUrl" : "\K[^"]*' \
| sort -V \
| tail -1)

FRONTEND_URL=$(curl -u "$USER:$PASS" -s -X GET \
"${URL}?repository=angular-app-repo" \
| grep -Po '"downloadUrl" : "\K[^"]*' \
| sort -V \
| tail -1)
